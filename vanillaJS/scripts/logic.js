...
class Wolf extends Player {
    ...
    act() {
        ...
        if (gm.preDoneFlg) {
            if (gm.isCheckCo(this.myPlayerCode) && this.lieJobCode === 'human1') { //既にCOしている
                utils.printMessage('さっきも言ったけど、' + jobSetting[this.lieJobCode] + 'です。', this);
            } else if (!gm.isCheckCo(this.myPlayerCode)) {
                this.lieJobCode = 'human1';
                utils.printMessage('CO村人です。', this);
    ...
    liePre() {
        ...
        if (0 == utils.getRand(0, 2) || !target.myJobCode.startsWith('wolf')) {
            utils.printMessage(`CO占い師、${target.myPlayerName}は人狼！`, this);
            super.predictResultSet(this, this.lieJobCode, target, 'wolf');
            target.reAct('wolf1');
        } else {
            utils.printMessage(`CO占い師、${target.myPlayerName}は白みたい`, this);
    ...
    reAct(doubtJob) {
        if (doubtJob.startsWith('wolf')) {
            utils.printMessage('ちがう！俺は' + jobSetting[doubtJob] + 'じゃねえ！', this);
            if (gm.isCheckCo(this.myPlayerCode)) {
                utils.printMessage('さっき言った通り、俺は' + jobSetting[this.lieJobCode] + 'だ！', this);
            } else {
                this.impersonate();
            }
        } else {
            if (gm.isCheckCo(this.myPlayerCode)) {
                utils.printMessage('そのとおり、俺は' + jobSetting[this.lieJobCode] + 'です。', this);
                utils.printMessage('さっき自分でCOしたし、信憑性高いね。', this.myPlayerName);
            } else {
                this.lieJobCode = 'human1';
                utils.printMessage('そのとおり、俺は' + jobSetting[this.lieJobCode] + 'です。', this);
                super.coResultSet(this, this.lieJobCode);
...
class Human extends Player {
    ...
    act() {
        ...
        if (gm.isCheckCo(this.myPlayerCode)) {
            utils.printMessage('さっきも言ったけど、村人です。', this);
        } else {
            utils.printMessage('CO村人です。', this);
    ...
    reAct(doubtJobCode) {
        if (doubtJobCode.startsWith('wolf')) {
            utils.printMessage('ちがう！俺は' + jobSetting[doubtJobCode] + 'じゃねえ！', this);

            if (gm.isCheckCo(this.myPlayerCode)) {
                utils.printMessage('さっき言った通り、俺は' + jobSetting[this.myJobCode] + 'だ！', this);
            } else {
                utils.printMessage('俺は' + jobSetting[this.myJobCode] + 'だ！', this);
                super.coResultSet(this, this.myJobCode);
            }

        } else { //White
            utils.printMessage('そのとおり。俺の役職は' + jobSetting[this.myJobCode] + 'です。', this);
            if (gm.isCheckCo(this.myPlayerCode)) {
                utils.printMessage('さっき自分でCOしたし、信憑性高いね。', this);
            } else {
                super.coResultSet(this, this.myJobCode);
            }
        }
...
class Pre extends Player {
    ...
    pre() {
        ...
        if (targetObj.myJobCode.startsWith('wolf')) {
            utils.printMessage('CO占い師、' + targetObj.myPlayerName + 'は人狼！', this);
            super.predictResultSet(this, 'pre', targetObj, 'wolf');
        } else {
            utils.printMessage('CO占い師、' + targetObj.myPlayerName + 'は白みたい', this);
    ...
    reAct(doubtJob) {
        if (doubtJob.startsWith('wolf')) {
            if (gm.isCheckCo(this.myPlayerCode)) {
                ...
                utils.printMessage('ちがう！俺は' + jobSetting[doubtJob] + 'じゃねえ！', this);
                utils.printMessage('こいつは偽物の占い師！俺が本物だ！俺の占い結果を発表する！', this);
                this.act();
            }
        } else {
            utils.printMessage('そのとおり、俺の役職は' + jobSetting[this.myJobCode] + 'です。', this);
...
class GameMaster {
    ...
    printJob() {
        let dispString = ''
        utils.printMessage('■ゲームがはじまります■');
        utils.printMessage('■今回の役職■');
        ...
        utils.printMessage(dispString);
    }

    //参加者の文字列表示
    printPlayer() {
        let dispString = ''
        utils.printMessage('■参加者の人たちです■');
    ...
    createPlayer() {
        ...
        for (let key in playerSetting) {
            ...
            switch (jobKeyArry[i]) {
                ...
                default:
                    utils.printMessage('おや、何かがおかしいようです。')
                    return false;
            }

            const introMessageObj = playerData[key]['intro'];
            utils.printMessage(introMessageObj[utils.getRand(0, Object.keys(introMessageObj).length)], this.playerInstance[i]);
            i++
        }
    ...
    playerAct() {
        if (gm.preDoneFlg) {
            utils.printMessage('■では村人は名乗り出てください■');
        } else {
            utils.printMessage('■昼がはじまります。占い師は名乗り出てください■');
        }
    ...
    createKillButton() {
        utils.printMessage('■いままでのやりとりから、誰が人狼でしょうか？■')
    ...
    judge(targetCode) {
        ...
        switch (target) {
            case target.startsWith('wolf') && target:
                utils.printMessage('処刑対象は人狼でした。あなたの勝利です。');
                break;

            case target.startsWith('human') && target:
                utils.printMessage('処刑対象は村人でした。あなたの敗北です。');
                break;

            case 'pre':
                utils.printMessage('処刑対象は占い師でした。あなたの敗北です。');
                break;

            case 'thief':
                utils.printMessage('処刑対象は怪盗でした。あなたの敗北です。');
                break;
    ...
    printFinalResult() {
        utils.printMessage('■役職は以下でした■')

        for (let i = 0; i <= this.playerInstance.length - 1; i++) {
            utils.printMessage(this.playerInstance[i].myPlayerName + ':' + jobSetting[this.playerInstance[i].myJobCode]);
        }
